...
SOCKET accept_tcp_conn(const int socket_server) {
    ...
    const SOCKET socket_client = accept(socket_server, (struct sockaddr *) &client, &client_len);
...
void recv_block(const SOCKET socket_client) {
    char buffer[BUFSIZ];

    ssize_t bytes_received = recv(socket_client, buffer, BUFSIZ, 0);

    if(bytes_received < 0) exit_with_sys_msg("recv() failed");

    while(bytes_received > 0){
        const ssize_t bytes_sent = send(socket_client, buffer, bytes_received, 0);

        if(bytes_sent < 0) exit_with_sys_msg("send() failed");
        else if (bytes_sent != bytes_received) exit_with_user_msg("send()", "sent unexpected number of bytes");
